...
- (void)viewDidLoad
{
    ...
    NSLog(@"correct id array from indanger %@",_inDangerCrrtIdArray);
    ...
    self.arrTestWords = [[DBHelper sharedDbInstance] fetchRecordFromDB:[NSString stringWithFormat:@"tbl_Dictionary WHERE id IN (%@)",idString] withField:@"targetWord, bucketColor,action,id"];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [SharedAppDelegate.tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"Test_Result" action:@"Test_Finish" label:@"WordBucket-Paid" value:nil] build]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (splitArray.count>2) {
        ...
        idString = [splitArray objectAtIndex:3];
    }
    
     UIImageView *tickImage = (UIImageView*)[cell.contentView viewWithTag:101];
    
    // Set bucket color from where word was
    
     UIImageView *firstBucket = (UIImageView*)[cell.contentView viewWithTag:103];
    [firstBucket setImage:_bucketImage];
    
    if ([self.lblColor isEqualToString:@"Green"] || [self.lblColor isEqualToString:@"InDanger"]){
        if ([_inDangerCrrtIdArray containsObject:idString])
            [firstBucket setImage:GetImage(@"iconDangerBucket")];
        else
    ...
    // Set bucket bucket from where word is belongs
...
- (void)showAlert
{
    ...
    /*
    if ([self.lblColor isEqualToString:@"InDanger"] || [self.lblColor isEqualToString:@"Green"])
        msgString = [self.lblColor isEqualToString:@"Green"] ? @"Sorry! To play the Green game you need at least 12 words in your Green Bucket. Move words here by testing your Orange words" : @"Sorry you need 12 words here before you can try to save your In Danger words.";
    else
        msgString = [NSString stringWithFormat:@"User must have minimum of %d words in their bucket to do any test.",SharedAppDelegate.totolQustCount];
    */
    
    //NSString *msgString = [NSString stringWithFormat:@"User must have minimum of %d words in their bucket to do any test.",questCount];
