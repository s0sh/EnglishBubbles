...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [SharedAppDelegate.tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"Orange_Test" action:@"Orange_Test_Begin" label:@"WordBucket-Free" value:nil] build]];
}
...
-(void) getNewQuestion
{
    ...
    if(quesNo <= SharedAppDelegate.totolQustCount)
    {
        ...
        if([arrNewWord count]>0)
        {
            ...
            {
                strCorrectAns = [arrCorrectWord objectAtIndex:0];
                NSLog(@"CORRECT ANS IS > %@",strCorrectAns);
                ...
                if (_isOrangeTest1) {
                    ...
                    [_lblAnswer setText:strCorrectAns];
...
-(IBAction)submitAnswer:(id)sender
{
    ...
    if (sender) {
        ...
        if (_isOrangeTest1) {
            ...
            {
                if([strCorrectAns isEqualToString:_txtNativeWord.text])
                {
...
- (IBAction)submitClicked:(id)sender {
    ...
    if( ([strCorrectAns caseInsensitiveCompare:_txtNativeWord.text] == NSOrderedSame) && ([sender tag] !=100)) {
        ...
        switch (_attemptForOT2) {
            ...
            {
                NSString *dotString = @"";
                for (int k =0; k < strCorrectAns.length; k++) {
                
                    unichar space = [strCorrectAns characterAtIndex:k];
            ...
            case 2:
            {
                ...
                for (int k =0; k < strCorrectAns.length; k++) {
                    ...
                    if(k==0){
                        dotString = [dotString stringByAppendingString:[strCorrectAns substringToIndex:1]];
                        continue;
                    }
                    unichar space = [strCorrectAns characterAtIndex:k];
                    if ([[NSString stringWithFormat:@"%c",space] isEqualToString:@" "])
                ...
                [_placeHolderLbl setText:dotString];
                [_txtNativeWord setText:[strCorrectAns substringToIndex:1]];
            ...
            case 3:
            {
                ...
                [_txtNativeWord setText:strCorrectAns];
                [_placeHolderLbl setText:strCorrectAns];
                [self pauseTimer];
...
- (void)textChanged:(NSNotification *)notification
{
    ...
    switch (_attemptForOT2) {
        ...
        case 2:
        {
            ...
            for (int k = _txtNativeWord.text.length; k < strCorrectAns.length; k++) {
                unichar space = [strCorrectAns characterAtIndex:k];
                if ([[NSString stringWithFormat:@"%c",space] isEqualToString:@" "])
                    dotString = [dotString stringByAppendingString:@" "];
                else
...
{
    if ([[UserDefaluts objectForKey:kNativeLangCode] isEqualToString:@"en"])
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (length >= strCorrectAns.length && ![string isEqualToString:@""]) {
        textField.text = [textField.text substringToIndex:strCorrectAns.length];
        checkContion =  NO;
    }
    }
    ...
    if (_attemptForOT2==2){
        int length = [textField.text length];
        NSLog(@"both lent is %d %d",length, strCorrectAns.length);
        if (length >= strCorrectAns.length && ![string isEqualToString:@""]) {
            textField.text = [textField.text substringToIndex:strCorrectAns.length];
            checkContion =  NO;
        }
