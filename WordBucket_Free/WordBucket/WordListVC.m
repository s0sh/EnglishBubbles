...
-(void) getWords
{
    ...
    for (int j=0;j<[arrSection count];  j++) {
        ...
        [self.arrForTable addObject:arrDict];
    }
    
   // NSLog(@"Main araay is %@",self.arrForTable);
    ...
    [(NSMutableArray*)self.arrForTable sortUsingDescriptors:sortDescriptors];
...
- (void)viewDidLoad
{
    ...
    self.arrForTable = [[NSMutableArray alloc] init];
    if (!expandedSections)
    {
...
- (void)reloadTable
{
    [self.arrForTable removeAllObjects];self.arrForTable = nil;
    self.arrForTable = [[NSMutableArray alloc] init];
...
{    
    ...
    if (title) {
        [Common showAlertView:@"" message:message delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
...
{
    NSMutableArray *wordsArray = [[self.arrForTable objectAtIndex:section] valueForKey:[NSString stringWithFormat:@"%d",section]];
...
{
     NSMutableArray *wordsArray = [[self.arrForTable objectAtIndex:indexPath.section] valueForKey:[NSString stringWithFormat:@"%d",indexPath.section]];
...
-(void)miniMizeThisRows:(NSArray*)ar section:(NSInteger)section row:(NSInteger) row{
	...
	for(NSMutableDictionary *strMean in ar ) {
        NSMutableArray *arrWords = [[self.arrForTable objectAtIndex:section] valueForKey:[NSString stringWithFormat:@"%d",section]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([dict valueForKey:@"Word"])
    {
		...
		for(NSMutableDictionary *strMeaning in arrMean ){
            NSMutableArray *arrWords = [[self.arrForTable objectAtIndex:indexPath.section] valueForKey:[NSString stringWithFormat:@"%d",indexPath.section]];
