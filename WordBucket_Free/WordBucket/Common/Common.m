...
static const unsigned long long kuFBAppID = 540867222608021;
...
- (void)synconCloud
{
    ...
    //NSLog(@"CECK SELF.DOC +++++ ++++ %@",self.doc);
    
    //NSLog(@"NOW DATA UPLOAD +++++ ++++ %@",allArray);
...
+(void)checkSessionAndStartIfNeeded:(void(^)(BOOL success))block
{
    ...
    if (FBSession.activeSession.isOpen) {
        ...
        /*[FBSession openActiveSessionWithReadPermissions:permissions
         ...
         if (error) {
         NSLog(@"Failed in opening Session");
...
+ (UIFont*)AdjustLabelFont:(UILabel*)label
{
    ...
    NSLog(@"font is %@",[font fontWithSize:finalFont]);
